...
public class TripwiseEarlyArrivalReport extends ActionSupport {
...
//	        System.out.println("#####");
	        System.out.println("parentId="+parentId+"formattedgivendate"+date);
	  ...
	  public String getTripwiseEarlyArrival(){
			
//			System.out.println("Enter into getTripwiseEarlyArrival");
			HttpServletRequest req = ServletActionContext.getRequest();
			ScheduleTripDurationDAO scheduleDAO=new ScheduleTripDurationDAO();
			
			try {
				CollectionReportDAO dao=new CollectionReportDAO();
//				System.out.println("Deport name:"+depotName);
...
//				
//				
//				String sql = "SELECT st.shift_type_name As shift_type_name,TRIP_NUMBER,VEHICLE_NO,END_BUS_STOP_NAME,END_TIME, ACT_END_TIME,TRIP_STATUS,"+
//                             " timestampdiff(minute,concat(date(ETM_END_TIME),' ',END_TIME),concat(date(ETM_END_TIME),' ',ACT_END_TIME)) diff"+
//                             " FROM `waybill_trip_details`wd "+
//                             " inner join  shift_type st on st.shift_type_id=wd.duty_type_id "+
//                             " WHERE DUTY_DT = '2017-01-02' AND SCHEDULE_NO='15692'  AND `ACT_END_TIME` IS NOT NULL AND `ACT_END_TIME` != '00:00:00' and wd.status !='NEW' "+
//                             " having diff < 15 limit 50 ;";
...
//					  		
//				
//				 Query query = session1.createSQLQuery(sql).addScalar("shift_type_name").addScalar("TRIP_NUMBER").addScalar("VEHICLE_NO").addScalar("END_BUS_STOP_NAME").addScalar("END_TIME").addScalar("ACT_END_TIME").addScalar("TRIP_STATUS").addScalar("diff");
//			query.setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE);
//				List<Map<String, Object>> aliasToValueMapList = query.list();
				
				VtsResponse6 alertresults=scheduleDAO.getDataTripWiseEarlyArrivalReport(date1,scheduleNo);
				
//				 System.out.println("alertresults Size==="+alertresults.getWaybillDetails().size());
				
				 regionTypeAjaxString +=" <div id='header' style='display: none;'><div align='center'><h4>"+""+" </br>"+ ""+" </br> Tripwise Early Arrival Report</br></h4></div>";
			     regionTypeAjaxString +="<div align='right'><b>Run Date:-</b>"+runDateTime+"<b>Schedule NO:-</b>"+scheduleNo+"</div></div>";
					...
					System.out.println("Reached here=="+alertresults.getWaybillDetails().size());
					 ...
					 for (int i = 0; i < alertresults.getWaybillDetails().size(); i++) {
							...
							int j=i+1;
						   System.out.println("Enter into forloop="+alertresults.getWaybillDetails().get(i).getShiftTypeName().toString());
