...
public class MissingEarlyArrival<ProcedureCall> extends ActionSupport {
...
//        	}
	...
	public String savemissingearly() {
		...
		Transaction trasation=session.beginTransaction();
		SQLQuery  query =(SQLQuery)session.createSQLQuery("CALL EarlyArrival(:depot_id,:run_date)").setParameter("depot_id",depot).setParameter("run_date",date1);
...
/*		 String rbKey = String.valueOf(getSysParameterForVts());
		 NewWsVts_Service service = new com.trimax.vts.NewWsVts_Service();
			...
			VtsResponseNew alertresult = port.insertearrlyarrival(depot, date1, rbKey);
        	...
        	if(result !=0){ ServletActionContext.getRequest().getSession().setAttribute("valueforactionmessage", "1");
  	       addActionMessage("Record inserted Successfully");
 		       ...
 		       addActionMessage(" No Data Avilable For Insert ");
        	}
		}catch (Exception e) {
			ServletActionContext.getRequest().getSession().setAttribute("valueforactionmessage", "2");
		       addActionMessage("Record not inserted ");
