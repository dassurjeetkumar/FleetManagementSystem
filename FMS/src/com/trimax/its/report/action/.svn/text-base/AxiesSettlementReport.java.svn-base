...
public class AxiesSettlementReport extends ActionSupport {
...
//		System.out.println("RouteNo==="+routeno);
		...
		/* if(Integer.parseInt(gprsvalue.toString()) >= Integer.parseInt(axiesvalue.toString()) ){*/
			...
			/* sql="SELECT org_name,eg.waybill_no,eg.schedule_no,eg.route_no,eg.trip_no,eg.etim_no,eg.ticket_date,eg.ticket_time,eg.ticket_no its_ticket,"+
			 ...
			 "if(sum(round((eg.px_total_amount+eg.lugg_total_amount),2)) =ifnull(ar.transaction_amount-ar.convenience_fee,0),'','NO') settlement "+
		 ...
		 sql="select org_name,waybill_no,schedule_no,route_no,trip_no,etim_no,ticket_date,ticket_time,its_ticket,axis_waybill,axis_ticket,"+
				 ...
				 "WHERE eg.depot_id!=42 and "+ 
				 "eg.ticket_date between '"+startdate1+"' AND '"+enddate2+"'  AND `payment_mode` = '1' group by eg.ticket_no,eg.waybill_no order by org_name, "+
				 "date(ticket_date),eg.ticket_no) a where settlement_status='NO'";
	/*	 }else {
			 sql=" SELECT org_name,eg.waybill_no,eg.schedule_no,eg.route_no,eg.trip_no,eg.etim_no,eg.ticket_date,eg.ticket_time,eg.ticket_no its_ticket,ifnull(ar.waybill_no,'') axis_waybill,ifnull(ar.ticket_id,'') axis_ticket, "+
				  " round((eg.px_total_amount+eg.convinence_fee),4) its_transactionAmt "+
				  " ,eg.convinence_fee its_convinence,ifnull(ar.transaction_amount,0) axis_transactionAmt,ifnull(ar.convenience_fee,0) axis_convinence,  "+
				  " if(round((eg.px_total_amount+eg.convinence_fee),2) =ifnull(ar.transaction_amount,0),'','NO') settlement "+
				  " FROM bmtcGprs.etim_gprs_ticket eg right join bmtcGprs.axis_reconcillation ar on eg.waybill_no=ar.waybill_no and eg.ticket_no=ar.ticket_id  "+
				  " inner join its.org_chart oc on eg.depot_id = oc.org_chart_id  "+
				  " WHERE ar.transaction_date between '"+startdate1+"' AND '"+enddate2+"' order by date(ticket_date),org_name";
		 }*/
