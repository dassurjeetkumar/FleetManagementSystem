...
public class ScheduleWisePercentETMAction {
	...
	public String getStartdate() {
		...
		public String getScheduleWisePercentETM()
		{
			try {
...
//				String depotdb=common.getDBResultStr(session1, sql1, "depotname");
			...
			String sql="select s.schedule_number,count(wd.schedule_No) noofdays,count(distinct(generated_Date)) ndays," +
					"sum(ETIM_Coll_Amt) etm,sum(manualpassenger) manual," +
					"round((sum(ETIM_Coll_Amt)/(sum(ETIM_Coll_Amt)+sum(manualpassenger)))*100) etmper,wd.schedule_No," +
					...
					"and (Bag_Coll_Amt+ETIM_Coll_Amt)>0 group by s.schedule_number order by manuper desc";
//        System.out.println(sql);
			 rs = stmt.executeQuery(sql);

		
			regionTypeAjaxString1.append("<div id='header' style='display: none;'><div align='center'> </br>Schedule Wise % Of ETM Usage Report </br>From Date:- "+startdate+" To Date:-"+ enddate+"</br></h4></div>");
			...
			regionTypeAjaxString1.append("<thead><tr><th>Sr No</th><th>Schedule No</th><th>No.Of Days Operated</th>" +
