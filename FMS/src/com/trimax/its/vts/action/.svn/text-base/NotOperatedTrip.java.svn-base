...
public class NotOperatedTrip extends ActionSupport {
	...
	VtsDataDAO dao = VtsDataDAO.getInstance();
		...
		public String getNotOperatedData() {
			...
			try {
				...
				result = getNotOpeartedDetails(startDate1,depot_id);
				out.print(result);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
		...
		public JSONObject getNotOpeartedDetails(String fromdate,String depot_id){
			JSONObject result = new JSONObject();
			try{
				...
				activeresult = port.webGetNotOperatedTripData(fromdate,depot_id);
