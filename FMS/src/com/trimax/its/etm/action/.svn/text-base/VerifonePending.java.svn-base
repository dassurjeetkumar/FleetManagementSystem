...
/*			String sql="select org_name,org_chart_id,"+
			...
			String sql="select oc.org_name,device_serial_number,ifnull(sd.phone_number,'') phone_number,"+
				...
				/*	"ifnull(etm_resolved_by_verifone,'')etm_resolved_by_verifone,ifnull(etm_received_by_fme,'')etm_received_by_fme,"+
					...
					"left join simcard sd on sd.simcard_id=sv.sim_id and sd.status='active' and sd.status='active' "+
				//	+ "left join etm_issue ei on ei.id=edh.etm_issue "+
					"where etm_received_date is  null and etm_resolved_by_fme is null and "
					+ "etm_pickup_by_verifone is not null and etm_pickup_by_fme is not null and "
					+ "etm_resolved_by_verifone is null and etm_received_date is null and "+
