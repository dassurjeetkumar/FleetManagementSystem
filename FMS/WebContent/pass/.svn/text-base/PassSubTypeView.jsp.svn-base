				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>Pass Sub Type
							</div>
...
<%-- 									<%if(edit.equalsIgnoreCase("Y")){ %> --%>
						...
						<div class="portlet-body">
							...
							<table class="table table-striped table-bordered table-hover"
								id="passsubtypeviewjs">
								<thead>
									<tr>
										<th style="width1: 8px;"></th>
										<th>Pass SubType Id</th>
										<th>Pass SubType Name</th>
	...
	function callEdit(){
		...
		if (cnt == 0) {
			...
			document.forms[0].action = "editpassSubType.action?passid="+ val;
...
function callDelete(){
		...
		if (cnt == 0) {
			...
			if(isEligibleForOpertion(val)){
			document.forms[0].action = "deleteSubTypeaction.action?passid="+ val;
			document.forms[0].submit();
			}else{
				bootbox.alert("Please Select Valid Record");
			}
	...
	function callCreate() {
		document.forms[0].action = "createPassSubTypeforPass.action";
		document.forms[0].submit();
	}
	
	$(document).ready(function() {
		   window.history.pushState("","", "PassSubType.action");
