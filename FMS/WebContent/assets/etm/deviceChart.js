		...
		initPieCharts : function() {
			...
			function pieClickalert(event, pos, obj) {
				...
				if(!$('input:radio[name=radio]:checked').val()){
				 ...
				 chat_type=$('input:radio[name=radio]:checked').val();
				}
				console.log(chat_type);
				console.log(obj.series.status);
				$("#Alldevice").attr("style", "display:none");
				if(obj.series.status=='Trimax'){
				  ...
				  $("#vstatus").html(
							"Device Data For" + "(" + obj.series.status + ")");
				  ...
				  table.dataTable({
						...
						"sAjaxSource" : "getDeviceDashboardData!getDeviceDetails?alertID="
								+ obj.series.status + "&deviceType=" + chat_type,
...
//					alert("------"+obj.series.status);
							  ...
							  table.dataTable({
								...
								jQuery(
										'#alert_dashBoard_data_wrapper .dataTables_length select')
										.addClass("form-control input-xsmall input-inline"); // modify
				}else if(obj.series.status=='ACTIVE'){
//					alert("------"+obj.series.status);
					  table = $('#alert_dashBoard_data');
					  divId = document.getElementById("vehicle");
					  $("#deviceGprs").attr("style", "display:none");
...
//			alert("------"+obj.series.status);
			  ...
			  table.dataTable({
				...
				jQuery(
						'#alert_dashBoard_data_wrapper .dataTables_length select')
						.addClass("form-control input-xsmall input-inline"); // modify
}else if(obj.series.status=='Int.Battary'){
//	alert("------"+obj.series.status);
	  table = $('#alert_dashBoard_data');
	  divId = document.getElementById("vehicle");
	  $("#deviceGprs").attr("style", "display:none");
	  ...
	  table.dataTable({
		...
		initNRDDetailsPieCharts : function() {
			function getNRDDetailsDashboard() {
				var vehicleData = $.ajax({
					url : "getDeviceDashboardData!getDVPPieChartDetails",
					dataType : 'json',
					contentType : "application/json",
					global : false,
					async : true,
					success : function(response) {
						var plotVehicleResponseData = response;
						plotNRDDetails(plotVehicleResponseData);
						return response;
					}
				}).responseText;
			}
			...
			;
			loadIntervalId = setInterval(function() {
				getNRDDetailsDashboard();
			}, 1000000);

			$("#alertspieNRDDETAILS").bind("plothover", pieHoveralert);
			$("#alertspieNRDDETAILS").bind("plotclick", pieClickalert);
			...
			function pieClickalert(event, pos, obj) {
				...
				console.log("click dvp--"+obj.series.status);
				$("#Alldevice").attr("style", "display:none");
//				if(obj.series.status=='Total Operable'){
//					var statusName="TotalOperable";
				  table = $('#dvp_dashboard_operable');
				  divId = document.getElementById("vehicle");
//				  $("#deviceGprs").attr("style", "display:none");
				  $("#vstatus").html(
							"Status : " + "(" + obj.series.status + ")");
//				  alert("come");
					$("#vehicle").attr("style", "display:none");
    				  $("#dvp").attr("style", "display:''");
				  table.dataTable({
						"aLengthMenu" : [ [ 5, 15, 20, -1 ],
								[ 5, 15, 20, "All" ] // change
						// per
						...
						// set the initial value
						"iDisplayLength" : 5,
						"sAjaxSource" : "getDeviceDashboardData!getDVPDeviceDetails?status_alertID="+ obj.series.status,
						"sPaginationType" : "bootstrap",
						"bDestroy" : true,
					...
					jQuery(
							'#dvp_dashboard_operable_wrapper .dataTables_filter input')
							.addClass("form-control input-small input-inline"); // modify
					// table
					jQuery(
							'#dvp_dashboard_operable_wrapper .dataTables_length select')
							.addClass("form-control input-xsmall input-inline"); // modify
					// table
...
//				}
				...
				/*else if(obj.series.status=='CWS'){
					  table = $('#dvp_dashboard_CWS');
					  divId = document.getElementById("dvp");
					  $("#dvp").attr("style", "display:''");
					  $("#vstatus").html(
								"Status :" + "(" + obj.series.status + ")");
					  table.dataTable({
							...
							// set the initial value
							"iDisplayLength" : 5,
							"sAjaxSource" : "getDeviceDashboardData!getDVPDeviceDetails?status_alertID="+ statusName+ "&deviceType=" + chat_type,
							"sPaginationType" : "bootstrap",
							"bDestroy" : true,
...
*/				
		...
		initNRDPieCharts : function() {
			function getDeviceNRDDataDashboard() {
				var vehicleData = $.ajax({
					url : "getDeviceDashboardData!getDevicePieChartNRDDetails",
					dataType : 'json',
					contentType : "application/json",
					global : false,
					async : true,
					success : function(response) {
						var plotVehicleResponseData = response;
						plotVehicleNRDData(plotVehicleResponseData);
						return response;
					}
				}).responseText;
			}
			...
			;
			loadIntervalId = setInterval(function() {
				getDeviceNRDDataDashboard();
			}, 1000000);

			$("#alertspieNRD").bind("plothover", pieHoveralert);
			$("#alertspieNRD").bind("plotclick", pieClickalert);
			...
			function pieClickalert(event, pos, obj) {
				...
				console.log(chat_type);
				console.log("click  "+obj.series.status);
				$("#Alldevice").attr("style", "display:none");
				$("#NRDDetails").attr("style", "display:none");
				if(obj.series.status=='CWS'){
				  ...
				  table.dataTable({
						...
						// set the initial value
						"iDisplayLength" : 5,
						"sAjaxSource" : "getDeviceDashboardData!getDeviceNRDDetails?alertID="
								+ obj.series.status + "&deviceType=" + chat_type,
						"sPaginationType" : "bootstrap",
						"bDestroy" : true,
				...
				}else if(obj.series.status=='OTHER'){
//					alert("------"+obj.series.status);
							  table = $('#alert_dashBoard_data');
							  divId = document.getElementById("vehicle");
							  $("#deviceGprs").attr("style", "display:none");
...
function getDeviceDataDashboard(devicetype) {
	
//	alert(devicetype);
	var vehicleData = $.ajax({
		url : "getDeviceDashboardData!getDevicePieChartDetails?deviceType=" + devicetype,
		dataType : 'json',
		contentType : "application/json",
		global : false,
		async : true,
...
function getDeviceNRDDataDashboard(devicetype) {
	
//	alert(devicetype);
	var vehicleData = $.ajax({
		url : "getDeviceDashboardData!getDevicePieChartNRDDetails?deviceType=" + devicetype,
		dataType : 'json',
		contentType : "application/json",
		global : false,
		async : true,
...
function getNRDDetailsDashboard(devicetype) {
...
function plotNRDDetails(vehicleData) {
	var data = [];
	// var series = Math.floor(Math.random() * 10) + 1;
	series = vehicleData["aaData"].length;
	...
	$('#totalDetailsNRD').text("Total Vehicles:"+vehicleData["aaData"][12]["0"]);
	$
			.plot(
					$("#alertspieNRDDETAILS"),
					data,
					{
						...
						colors : [ "#33ffbe", "#C70039", 
								, " #bb33ff",  "#ff3399","#ff6932","#f1ff31","#404f4c" ,"#ff5733", "#ff8d33","#FFD4A4","#41423c","#a5ff33"],
						grid : {
							hoverable : true,
							clickable : true
						}
...
function plotVehicleNRDData(vehicleData) {
	var data = [];
	// var series = Math.floor(Math.random() * 10) + 1;
	series = vehicleData["aaData"].length;
	...
	$('#totalVehicleNRD').text("Total Devices:"+vehicleData["aaData"][3]["0"]);
	$
			.plot(
					$("#alertspieNRD"),
					data,
