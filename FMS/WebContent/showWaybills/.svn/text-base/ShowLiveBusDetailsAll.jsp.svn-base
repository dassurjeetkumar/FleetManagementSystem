...
<script>
	...
	function getCordinatesvalue(value) {
		//alert(value);
 		//var divId = document.getElementById("mapdiv");
		//divId.style.display = '';
		initialize();
		//initializeMap();
	 ...
	 ///  alert("vehicleID"+vehicleID);
	   ...
	   //alert("enddate"+enddate);
		...
		//alert("Please select"+divisionlist);
// 		if(divisionlist==0){
...
// 		}
		if (vehicleID == 'ALL') {
			...
			/* var divId2 = document.getElementById("end"); */
			...
			getCordinates(vehicleID, 0, startdate, enddate,"");
			$("#msg").html("");
		}
	}
...
// 		}
		...
		if (vehicleID == 'ALL') {
			...
			getCordinatespb(vehicleID, 0, startdatee, enddat,value,timeframe);
			$("#msg").html("");
		}
	}
...
<%-- 								id="startdate" class="form-control" value='<s:property value="ticketDate"/>'> <span --%>
...
<!-- 					<label class="control-label col-md-2">End Date/ Time </label> -->
					<div class="col-md-4">
<!-- 					<div class="input-group date form_datetime"> -->
<%-- 						<input type="text" size="16" readonly name="enddate" id="enddate" value='<s:property value="ticketDate"/>' --%>
