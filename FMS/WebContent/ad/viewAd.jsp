...
accessrights=accessrightdao.accessRightsdetails(usrsessionid, "viewAdMannagement.action");
...
<%if(delete.equalsIgnoreCase("Y")){ %>
								...
								<%}%>
								 ...
								 <a href="#" class="btn green" id="getAllAddsForCustomer" onclick="getAllAdds()">
									<i class="fa fa-pencil"></i>Get All Ads 
								</a>
						...
						<input type="hidden" name="requestType" id="requestType" value="text"/>
							<table class="table table-striped table-bordered table-hover" id="viewAd">
...
/* 	 $(document).ready(function() {
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you sure you want to delete?",
					function(result)
					{
						...
						if (result == true) 
						{
			document.forms[0].action = 'deleteAdMannagement.action?addid='
				+ val;
	...
	function editDisplayMapping(){
				...
				if (cnt == 0) {
					...
					//alert(val);
					if(isEligibleForOpertion(val)){
					document.forms[0].action = 'editAdMannagement.action?id='+ val;
					document.forms[0].submit();
					}else{
						bootbox.alert("Please Select Valid Record");
					}
	 ...
	 {
		 document.forms[0].action = "createAdMannagement.action";
		 document.forms[0].submit();
	 }
	 function getAllAdds()
	 {
		 document.forms[0].action = "getAllAdds.action";
		 document.forms[0].submit();
	 }
