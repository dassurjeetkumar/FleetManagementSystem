				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>View Vehicle Defect 
							</div>
							<div class="actions">
							...
							<%}%>
								<%if(delete.equalsIgnoreCase("Y")){ %>
								 <a href="javascript:void(0)" class="btn red" id="deletedefect" onclick="calldelete()">
							...
							<div class="btn-group">
									...
									<%if(status.equalsIgnoreCase("ACTIVE")){ %>
										<li>
											<a class="btn blue" href="javascript: void(0)" onclick="window.open('','_new').location.href='/its/generateReport?id=VEHICLEDEFECTSRPT'">
						...
						<div class="portlet-body">
							...
							<s:if test="%{deletestatus=='fail'}">
								Could Not Delete Vehicle Defect Deleted 
							</s:if>
							</div>
							<table class="table table-striped table-bordered table-hover"
								id="listvehicledefect">
								<thead>
									<tr>
...
<!-- 										<th>Registration Number</th> -->
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you Sure, you want to delete this record?",
							function(result) {
								...
								if (result == true) {
									document.getElementById("deldefectid").value = val;
	...
	function callEdit() {
		...
		if (cnt == 0) {
			...
			if(isEligibleForOpertion(val)){
			document.getElementById("editdefectid").value = val;
		...
		function callCreate() {
			document.forms[0].action = "createVehicleDefectsview.action";
			document.forms[0].submit();
		}
		$(document).ready(function() {
			   window.history.pushState("","", "vehicledefectlist.action");
			 });
...
<form name="form1" id="form1" action="editVehicleDefectDetails.action" method="POST">
	<input type="hidden" name="editdefectid" id="editdefectid" value="" />

</form>
<form name="form2" id="form2" action="DeleteVehicleDefects.action"
	method="POST">
	<input type="hidden" name="deldefectid" id="deldefectid" />
</form>
<%}else{%>
