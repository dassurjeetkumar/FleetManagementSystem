...
<script>
	function callEdit() {
		...
		//$(function() {
			...
			if (check(0)) {
				...
				document.getElementById("editMakeTypeId").value=val;
	...
	function callCreate() {
		document.forms[0].action = "CreateMakeType.action";
	...
	function callDelete() {
		...
		//$(function() {
			...
			if (check(0)) {
				if(isEligibleForOpertion(val)){
				bootbox	.confirm("Are you sure you want to delete Make Type?",function(result) {
					if (result == true) {
						document.getElementById("makeTypeId").value=val;
	...
	function check() {
		...
		if (chckbxCount > 0 && chckbxCount > 1) {
			bootbox.alert("Select only one Make Type...!")
			return false;
		} else if (chckbxCount > 0 && chckbxCount == 1) {
			return true;
		} else {
			bootbox.alert("Please select Make Type");
			return false;
		}
...
%>
<form name="editAction" id='editAction' action="EditMakeType.action" method="post">
	<input type="hidden" id=editMakeTypeId name="editMakeTypeId"/>
</form>
<form name="deleteAction" id="deleteAction" action="DeleteMakeType.action" method="post">
	<input type="hidden" id='makeTypeId' name="makeTypeId"/>
</form>
				...
				<div class="portlet box grey-cascade">
					<div class="portlet-title">
						<div class="caption">
							...
							<i class="fa fa-globe"></i> View Make Type
						</div>
						<div class="actions">
...
<%}%>
                      ...
                      <button class="btn blue dropdown-toggle"    data-toggle="dropdown"><i class="fa fa-gears"></i> Tools <i class="fa
                                    ...
                                    <a class="btn blue" href="javascript: void(0)"onclick="window.open('','_new').location.href='/its/generateReport?id=MAKETYPERPT'">
...
<%}%>
							...
							<%-- 	<tbody>
									<s:if test="%{MakeTypeList.isEmpty}%"></s:if>
									<s:else>
										<s:iterator value="MakeTypeList">
											<tr>
												<td><input type="checkbox" class="checkboxes"
													value="<s:property value="Make_type_id" />" /></td>
												<td><s:property value="Make_type_id" /></td>
												<td><s:property value="Make_type_name" /></td>
...
$(document).ready(function() {
	window.history.pushState("","", "MakeTypeList.action");
