...
accessrights=accessrightdao.accessRightsdetails(usrsessionid, "viewPisScrollMessage.action");
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
		 					<div class="caption">
								<i class="fa fa-globe"></i>View Scroll Message
							</div>
							<div class="actions">
...
<%if(delete.equalsIgnoreCase("Y")){ %>
						...
						<input type="hidden" name="requestType" id="requestType" value="text"/>
							<table class="table table-striped table-bordered table-hover" id="viewScrollMessage">
...
/* 	 $(document).ready(function() {
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you sure you want to delete?",
					function(result)
					{
						...
						if (result == true) 
						{
			document.forms[0].action = 'deleteScrollMessage.action?scrollmessgid='
				+ val;
	...
	function editDisplayMapping(){
				...
				if (cnt == 0) {
					...
					//alert(val);
					if(isEligibleForOpertion(val)){
					document.forms[0].action = 'editPisScrollMessage.action?id='+ val;
					document.forms[0].submit();
					}else{
						bootbox.alert("Please Select Valid Record");
					}
	 ...
	 {
		 document.forms[0].action = "createPisScrollMessage.action";
		 document.forms[0].submit();
	 }
