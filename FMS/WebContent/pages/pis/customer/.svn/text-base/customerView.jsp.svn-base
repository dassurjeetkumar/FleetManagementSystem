...
<%if (access.equalsIgnoreCase("Y")){%>
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>View PIS Customer 
							</div>
							<div class="actions">
							<%if(create.equalsIgnoreCase("Y")){ %>
								<a href="#" class="btn green" id="addCustomer"
									onclick="callCreate()"> <i class="fa fa-plus"></i> Create
								</a> 
								<%}%>
                  <%if(edit.equalsIgnoreCase("Y")){ %>
                  <a href="#" class="btn blue" id="editCustomer"> <i class="fa fa-pencil"></i>
					Edit
				  </a>
								<%}%>

                    <%if(delete.equalsIgnoreCase("Y")){ %> <a href="#" class="btn red" id="deleteCustomer"
								...
								<div class="btn-group">
									...
									<ul class="dropdown-menu pull-right ">
										<li><%if(status.equalsIgnoreCase("ACTIVE")){ %>
											<a class="btn blue" href="javascript: void(0)" onclick="window.open('','_new').location.href='/its/generateReport?id=PISCUSTOMERRPT'">
	...
	<script>
			...
			function callCreate() {
				document.forms[0].action = "AddCustomerAction.action";
				document.forms[0].submit();
			}
			
			$(document).ready(function() {

				window.history.pushState("", "", "customerView.action");
...
<form name="form1" id="form1" action="DeleteCustomerAction.action" method="POST">
 	<input type="hidden" name="cid" id="did" value="" /> 

 </form>
<form name="form2" id="form2" action="EditCustomerAction.action" method="POST">
