...
function getStageWiseData(){
	...
	var routeId=$("#route").val();
        $.ajax({
            ...
            type: "post",
            url: '<%=request.getContextPath()%>/getStagewiseTicketConsumptionData.action?date='+dd1+'&routeId='+routeId,
            success: function(result) {
            	$('#pageLoader').hide();
...
alert(val);
    $.ajax({
        type: "post",
        url: '<%=request.getContextPath()%>/getScheduleDetails?route='+val,
		success : function(result) {
...
function getDepot(){
		...
		var dd1=$("#startdate").val();
		var dd2=$("#endate").val();
 		var scheduleRange=document.getElementById("scheduleid").value;
 		//alert(servtype);
        ...
        $.ajax({
            ...
            type: "post",
            url: '<%=request.getContextPath()%>/getNrdVehicleData.action?enddate='+dd2+'&schedule='+scheduleRange+'&startdate='+dd1,
            success: function(result) {
            	$('#pageLoader').hide();
                document.getElementById('ticketconsuptionid').innerHTML=result;
                fixHeader();
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>StageWise Ticket Consumption Report
							</div>
						</div>
...
<!-- 								</SCRIPT> -->
						...
						<%-- <div class="form-group">
							...
							<div align="center">
					<button type="submit" class="btn green" onClick="getStageWiseData()">Submit</button>
