				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>View Holiday Type 
							</div>
							<div class="actions">
...
<!-- 								</a> -->
							...
							<div class="btn-group">
									...
									<ul class="dropdown-menu pull-right ">
										...
										<%if(status.equalsIgnoreCase("ACTIVE")){ %>
											<a class="btn blue" href="javascript: void(0)" onclick="window.open('','_new').location.href='/its/generateReport?id=HOLIDAYTYPERPT'">
						...
						<div class="portlet-body">
							...
							<s:if test="%{deletestatus=='fail'}">
								Could Not Delete Holiday Type Deleted 
							</s:if>
							</div>
...
<%}%>
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you Sure, you want to delete this record?",
							function(result) {
								...
								if (result == true) {
									document.getElementById("delholidaytypeid").value = val;
	...
	function callEdit() {
		...
		if (cnt == 0) {
			...
			if(isEligibleForOpertion(val)){
			document.getElementById("editholidaytypeid").value = val;
		...
		function callCreate() {
			document.forms[0].action = "createholidaytype.action";
			document.forms[0].submit();
		}
		$(document).ready(function() {
			   window.history.pushState("","", "HolidayTypelist.action");
			 });
...
<form name="form1" id="form1" action="ModifyHolidayTypeDetails.action" method="POST">
	<input type="hidden" name="editholidaytypeid" id="editholidaytypeid" value="" />

</form>
<form name="form2" id="form2" action="DeleteHolidayType.action"
	method="POST">
	<input type="hidden" name="delholidaytypeid" id="delholidaytypeid" />
</form>
