...
<script src="https://maps.googleapis.com/maps/api/js?client=gme-trimaxitinfrastructure&sensor=false&libraries=places,drawing&signature=CpSOeA5B6SzC2rxqlzd2JwenGX0="></script>
	<script src="assets/admin/pages/scripts/edit_busstopgroup.js" type="text/javascript"></script>
								...
								<div class="panel panel-default" >
									...
									<div class="panel-collapse collapse in" id="collapse_3_1"  style="color:#000000;overflow:auto;">
										<div class="panel-body">
											...
											<td style="display:none"><input type="text" id="busstopgroupid" value=""></td>
											<td>
											<select class="form-control input-sm" id="group_type"
											name="group_type" disabled>
...
<!-- 											<tr id="showgroupname"><th align="right">Group Name:<font color="red">*</font></th><td><input class="form-control input-sm" placeholder="" type="text" id="group_name" name="group_name" maxlength="100" readonly></td></tr> -->
											...
											<th colspan="2">Group Bus Btop List View</th>
											<tr><th>Bus Stop Name</th><th></th></tr>
											</table>
											</td>
											</tr>
											</table>	
...
function getBusStopGroupData(){
	$.ajax({
	    ...
	    url         :  "<s:url action='GetBusStopGroupEditdata'/>",
	   
	    success: function(data){
	        //alert('successful'+data);
	        var bussopslist="";
	        result=JSON.parse(data);
	        var selid=["group_type"];
	        var seval=[result.busstopgroupdata[0][4]];
	        $("#group_name").val(result.busstopgroupdata[0][5]);
	        $("#busstopgroupid").val(result.busstopgroupdata[0][0]);
	        makeSelected(selid,seval);
	        $("#bus_stop_name").val(result.busstopgroupdata[0][2]);
	        $("#start_stop_id").val(result.busstopgroupdata[0][1]);
	        if(result.busstopgroupdata[0][3]=="1"){
	    		$('#showgroupname').show();
	    		$('#showbusstopname').hide();
	    	}else{
	    		$('#showgroupname').hide();
	        ...
	        var showbutton=0;
	        for(var i=0;i<result.busstopgroupStopsize;i++){
	        	/* if(i==0){
	        		alert("HIII"+result.busstopgroupdata[0][6]+"====="+result.busstopgroupdata[0][7]);
	        		map.setCenter(new google.maps.LatLng(result.busstopgroupdata[0][6],result.busstopgroupdata[0][7]));
	        	} */
	        	showbutton=0;
	        	if(result.busstopgroupdata[0][3]=="2"){
	        		if(result.busstopgroupdata[0][1]==result.busstopgroupStopdata[i][0]){
	        			showbutton=1;
	        		}
	        	}
	        	if(result.busstopgroupStopdata[i][2]=="N"){
	        		showbutton=1;
	        	}
	        	if(showbutton==0){
	        	$('#listbusstop tr').last().after('<tr><td class="srno"></td><td>'+result.busstopgroupStopdata[i][1]+'</td><td class="busstopid" style="display:none">'+result.busstopgroupStopdata[i][0]+'</td><td><button class="close" aria-hidden="true" data-dismiss="alert" type="button" onclick="removerow('+result.busstopgroupStopdata[i][0]+')" ></button></td></tr>');
	        	 }else{
	        		$('#listbusstop tr').last().after('<tr><td class="srno"></td><td>'+result.busstopgroupStopdata[i][1]+'</td><td class="busstopid" style="display:none">'+result.busstopgroupStopdata[i][0]+'</td><td></td></tr>');	
	        	} 
	        	bussopslist=bussopslist+result.busstopgroupStopdata[i][0]+",";
	        	putdata(result.busstopgroupStopdata[i][0]);
	        	//alert(result.busstopgroupStopsize-1);
		        if(i==result.busstopgroupStopsize-1){
						//alert("hiiii");
...
function makeSelected(id,value) {
  ...
  //  alert(sel.value+"====="+sel.toSource());
    for(var i = 0, j = sel.options.length; i < j; ++i) {
    	//alert("hiii"+sel.options[i].innerHTML);
...
function submitAjaxForm(){
	...
	var group_name="";
	var start_stop_id="";
	var groupid=$('#busstopgroupid').val();
	if(group_type_id==1){
...
$.ajax({
    ...
    url         :  "<s:url action='EditBusStopGroupSave'/>",
   
    success: function(data){
    	
    	if(data=="Saved Successfully"){
    		if(group_type_id==1){
    			alert("Bus Stop Group "+group_name+" Edited Successfully");
    		}else{
    			alert("Trip Mapping for "+$('#bus_stop_name').val()+" Edited Successfully");
    		}
