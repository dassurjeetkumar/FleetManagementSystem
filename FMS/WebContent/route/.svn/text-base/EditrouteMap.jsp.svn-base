...
<script src="assets/admin/pages/scripts/edit_route.js" type="text/javascript"></script>
 	...
 	<div id="pageLoader" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel3" aria-hidden="false">
		...
		<div class="portlet box white" >
						...
						<div class="portlet-body" >
							...
							<input type ="checkbox" name="routeplatid" id="routeplatid">EDIT PLATFORM ONLY
							</h4>
							</div>
							</div> -->
...
function addstop(id,trid, busstationid, name){
	//alert("Route No=>"+$('#route_number').val()+" "+$('#route_direction').val()+"Bus Stop Name=>"+name);
	//alert(tableplatarr.toSource());
	$('#routebusstop').val("Route No=>"+$('#route_number').val()+" "+$('#route_direction').val()+"Bus Stop Name=>"+name);
	$('#routebusstop').html("Route No=>"+$('#route_number').val()+" "+$('#route_direction').val()+" ,    Bus Stop Name=>"+name);
	$('#trid').val(trid);
  ...
  // alert("hiiiiii"+tableplatarr);
	for(s=0;s<tableplatarr.length;s++){
		...
		$('#platform_details tr').last().after('<tr><td colspan="1">'+tableplatarr[s].planame+'</td><td>'+tableplatarr[s].sername+'</td><td><input type="hidden" class="depot" id="depot'+uniqueid+'" value="'+tableplatarr[s].depot+'"></td><td><input type="hidden" class="floor" id="floor'+uniqueid+'" value="'+tableplatarr[s].floor+'"></td><td><input type="hidden" class="bay" id="bay'+uniqueid+'" value="'+tableplatarr[s].bay+'"></td><td><input type="hidden" class="platform" id="platform'+uniqueid+'" value="'+tableplatarr[s].platform+'"></td><td><input type="hidden" class="service" id="service'+uniqueid+'" value="'+tableplatarr[s].service+'"></td><td style="display: none;">'+busplatid+""+""+tableplatarr[s].platform+""+tableplatarr[s].service+'</td><td><button class="close" aria-hidden="true" data-dismiss="alert" type="button" onclick="removeplatform('+busplatid+""+tableplatarr[s].platform+""+tableplatarr[s].service+""+',\''+busplatid+"|"+tableplatarr[s].platform+"|"+tableplatarr[s].service+'\')"></button></td></tr>');
		}
		}
	}
 		...
 		for(var i=0;i<platformsize;i++){
 			...
 			$("#servicetype-"+ platformnos[i] +" :selected").each(function(){
 			    ...
 			    //   tableplatarr.push({id:busstopd,planame:platform,sername:$(this).html()});
 			      tableplatarr.push({id:busstopd,name:busplatid+""+platformnos[i].replace(",", "")+""+$(this).val()+"",planame:platform,sername:$(this).html(),   depot:depotname,floor:floorname,bay:bayname,platform:platformnos[i],service:$(this).val()});
 			     var temp="";
...
//var routeplatid=$('#routeplatid').is(":checked");
	
	/* var routeplatidval=0;
	if(routeplatid){
		routeplatidval=1;
	}else{
		routeplatidval=0
	...
	//alert(routeplatidval);
	//return false;
	var editscheduleflag=0;
	$.ajax({
	    type        : 'POST',
	    data:  '&route_id='+route_id+
	    	   '&busstopid_P='+busstopid_P
		,
	    url         :  "<s:url action='GetScheduleData'/>",
	    ...
	    success: function(data){
	    	...
	    	if(data!=""){
	    		...
	    		if(data[0]==1){
	    			var retVal;
	    			/* if(data[1]!=""){
	    		   retVal = confirm("Route No. "+route_number+" is Used In "+ data[1]+" Schedule. Do you want to update Edited Route in Mentioned Schedule ?");
	    			}
	    			else{
	    				retVal=false;
	    			}
	    		   ...
	    		   if( 0==0  ){
	    			   $.ajax({
	    				    ...
	    				    '&servicepl='+ servicepl+
	    				    '&busplatformidpl='+busplatformidpl+
	    				    '&editscheduleflag='+editscheduleflag
	    					,
	    				    ...
	    				    success: function(data){
	    				    	...
	    				    	if(result[1]=="Route Created Successfully"){
	    				    		...
	    				    		if(typeof result[1]!='undefined'){
	    				    			...
	    				    			alert("Database Error..Retry After Some Time.");
	    				    		}
	    				    ...
	    				    error : function(xhr, errmsg) {
	    				    	$('#done').prop("disabled", false); 
	    				    	alert("Database Error. Please Login Again.");
	    				    }
	    				});
	    				
	    			   }else{
	    			      alert("Route Not Edited");
	    			      $('#done').prop("disabled", false); 
	    				  return false;
	    			   }
	    		}
