...
<script src="assets/admin/pages/scripts/create_route.js" type="text/javascript" value='1'></script>
	...
	$.ajax({
	    ...
	    success: function(data){
	  	...
	  	//alert('successful'+result);
	        initialize(result,loc,zoom,"true");
	    },
...
/* function add_platform(){
	...
	for(var m=0;m<uniqueid;m++){
		...
		$('#platformdetail tr').last().after('<tr id="1">          <td><input type="hidden" id="busplatformid'+platformcounter+'" value="'+busplatid+'">    </td>              <td><input type="hidden" id="depotp'+platformcounter+'" value="'+$('#depot'+m).val()+'">    </td><td><input type="hidden" id="floorp'+platformcounter+'" value="'+$('#floor'+m).val()+'"></td>    </td><td><input type="hidden" id="bayp'+platformcounter+'" value="'+$('#bay'+m).val()+'"></td>    </td><td><input type="hidden" id="platformp'+platformcounter+'" value="'+$('#platform'+m).val()+'"></td>    </td><td><input type="hidden" id="servicep'+platformcounter+'" value="'+$('#service'+m).val()+'"></td></tr>');
	...
	$.ajax({
	    ...
	    success: function(data){
	       ...
	       // session.removeAttribute("centerlange");
	        %> 
	        //alert(clat+"===="+clang);
	        initialize(result,center,13,"false");
	    },
	    error : function(xhr, errmsg) {
	    	//alert("Session out error..Please login again");
	    	}
	   ...
	   /*  data: {
	    ...
	    success: function(data){
	       // alert('successful'+data);
	        ...
	        //alert(idval);
	        initialize(result,center,13,"true");
	    },
	    error : function(xhr, errmsg) {
	    	//alert("Session out error..Please login again");
	    	}
...
function submitAjaxForm(){
	...
	for (var i = 0; i < sorted_arr.length - 1; i++) {
		if (sorted_arr[i + 1] == sorted_arr[i]) {
			alert("Duplicate Bus Stop "+full_busstopname[i]+" In Route Creation");
			$('#done').prop("disabled", false); 
		return false;
		}
	}
