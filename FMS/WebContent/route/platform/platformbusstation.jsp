...
function loadbayOnpgLoad(){
    ...
    $.ajax({
        type: "post",
        url: '<%=request.getContextPath()%>/PlatformAction!getBayName?orgid='
								+ val,
						success : function(result) {
							document.getElementById('bay_id').innerHTML = result;
						}
...
<s:select list="floorNameMap" 
												id="floor_id"
												name="platform.bay.floor.floor_id"
												onchange="loadbay(this.value)"
		...
		cssClass="select2_category form-control" headerKey="0" headerValue="Select"></s:select>
		 --%>
											<select class="form-control" id="bay_id"
												...
												onchange="getPlatForm(this.value);">
								...
								<div class="portlet box blue" id="platformdiv"
									...
									<div class="portlet-body">
										...
										<div id="platformMoreDetails" style="visibility: hidden">
										<div id="busname">
	...
	var oTable;
	function getPlatForm(busstation_idd) {
		var div = document.getElementById('platformdiv');
		div.style.visibility = 'visible';
		document.getElementById('platformMoreDetails').style.visibility = 'visible';
		...
		document.getElementById('busname').innerHTML="<B>"+parentID+"-->"+floorId+"-->"+bayId+"</B>";
		var update_id = "";
		var new_id = 0;
		var edit_id=0;
		var insert_id = "";
							...
							.ajax(
									{
										...
										url : "platFormAjaxAction!deleterows?id="
												+ delete_id,
		...
		$('#platformDatatable a.cancel').unbind().live('click', function(e) {
			e.preventDefault();
			var $this = $(this);
