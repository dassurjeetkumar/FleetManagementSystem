				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>Route List
							</div>
							<div class="actions">
							...
							<%if(edit.equalsIgnoreCase("Y")){ %>
								...
								<%}%>
								<%if(edit.equalsIgnoreCase("Y")){ %>
								<a href="#" class="btn blue" onclick="callEditRouteName()">
								<i class="fa fa-pencil"></i>EditRouteName</a>
								<%}%>
								...
								<%}%>
								<a href="#" class="btn green" onclick="tabletoExcel()"> <i
								class=""></i> Export
							</a>					
							</div> 
							</div>
...
<s:if test="hasActionMessages()">
					<font color="green"> <s:actionmessage />
						...
						<div class="table-scrollable" id= "routeid">
							<table class="table table-striped table-bordered table-hover" id="route">
							<thead >
...
function callEditRouteName(){
			
			var cnt = $(":checkbox:checked").length;
			var val;
			if (cnt == 0) {
				bootbox
						.alert("Please Select Checkbox To Route Name Edit");
			} else if (cnt > 1) {
				bootbox
						.alert("Please Select One Checkbox To Edit");
			} else {
				...
				document.forms[0].action = "editRouteName.action?routeid="+ val;
				document.forms[0].submit();
		}
		}callEditRouteName
	...
	function callajax(){
		...
		<script>                 	
   	        	function tabletoExcel(){      
   	        		alert("display here"+htmltable);
//    	           	var htmltable= document.getElementById('btnExport');    
   	            $( "#header-fixed" ).remove();
   	            var inputHTML = "<table border='1'>";
   	            inputHTML += "<tr>";
   	            inputHTML += "<th  align='left'colspan='14'>Bangalore Metropolitan Transport Corporation</th>";
   	            inputHTML += "</tr>";
   	            inputHTML += "<th  align='left'colspan='14'>Toll ticketing</th>";
   	            inputHTML += "</tr>";
   	            inputHTML += "<tr>";
//    	            inputHTML += "<th colspan='7' align='left'>Date Range:</th>";
//    	            inputHTML += "<th colspan='7' align='left'>" + $("#startdate").val() + " to " + $("#endate").val() + "</th>";
   	            inputHTML += "</tr>";
   	            inputHTML += "</table>";
   	      
                var htmltable = document.getElementById("routeid"); 
                alert("display here"+htmltable);
   	            var html = inputHTML +  htmltable.outerHTML; 
   	            var downloadLink = document.createElement("a"); 
   	            downloadLink.href = 'data:application/vnd.ms-excel,' + encodeURIComponent(html); 
   	            downloadLink.download = "Route details.xls";
