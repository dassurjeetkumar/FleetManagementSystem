...
function callEdit() {
	...
	if (cnt == 0) {
		...
		document.getElementById("ratemasterid").value = val;
	...
	if (cnt == 0) {
		...
		bootbox.confirm("Are you sure, you want to delete this record?",function(result) {
							...
							if (result == true) {
								...
								document.getElementById("delratemasterid").value = val;
...
function callCreate() {
	document.forms[0].action = "createReportMaster.action";
	document.forms[0].submit();
}

function callEmailConfiguration(){
	var cnt = $(":checkbox:checked").length;
	var val;
	if (cnt == 0) {
		bootbox.alert("Please Select Report Master");
	} else if (cnt > 1) {
		bootbox.alert("Please Select One Checkbox To Edit");
	} else {
		$("input[type='checkbox']:checked").each(function() {
		...
		if(isEligibleForOpertion(val)){
		document.getElementById("deratemasterid").value = val;
...
$(document).ready(function() {
	...
	//window.history.pushState("", "", "viewPisTemplate.action");
...
<div class="page-content-wrapper">
			...
			<div class="col-md-12">
				...
				<div class="portlet box grey-cascade">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-globe"></i>View Report Master
						</div>
						<div class="actions">
	...
	<%if(status.equalsIgnoreCase("ACTIVE")){ %>
								...
								<div id="sample_4_column_toggler" class="dropdown-menu hold-on-click dropdown-checkboxes pull-right">
									...
									<label>
										<button type="button" class="btn blue" onclick="callEmailConfiguration();">
					...
					<div class="portlet-body">
						...
						<table class="table table-striped table-bordered table-hover"
							...
							<s:if test="%{insertstatus=='fail'}">
								<font color="green">PIS Template could not Inserted
									Please Try after Sometime!!</font>
							</s:if>
							<s:if test="%{deletestaus=='fail'}">
								<font color="red">PIS Template could not Deleted Please
							...
							<thead>
								<tr>
									...
									<th>Report Master Id</th>
									<th>Report Master Name</th>
									<!-- <th>Notes</th> -->
									<th>Status</th>
...
<form name="form1" id="form1" action="editReportMaster.action"
	method="POST">
	<input type="hidden" name="ratemasterid" id="ratemasterid" value="" />
</form>
<form name="form2" id="form2" action="deleteReportMaster.action"
	method="POST">
	<input type="hidden" name="delratemasterid" id="delratemasterid" />
</form>
<form name="form3" id="form3" action="emailConfigurationView.action"
	method="POST">
	<input type="hidden" name="deratemasterid" id="deratemasterid" />
</form>
