...
<%if (access.equalsIgnoreCase("Y")){ %>
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>View Data Card 
							</div>
							<div class="actions">
							...
							<%}%>
								<%if(delete.equalsIgnoreCase("Y")){ %>
								 <a href="javascript:void(0)" class="btn red" id="deletedatacard" onclick="calldelete()">
								...
								<div class="btn-group">
									...
									<ul class="dropdown-menu pull-right ">
										...
										<%if(status.equalsIgnoreCase("ACTIVE")){ %>
											<a class="btn blue" href="javascript: void(0)" onclick="window.open('','_new').location.href='/its/generateReport?id=DATACARDRPT'">
						...
						<div class="portlet-body">
							...
							<s:if test="%{deletestatus=='fail'}">
								Could Not Delete Data Card Deleted 
							</s:if>
							</div>
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you Sure, you want to delete this record?",
							function(result) {
								...
								if (result == true) {
									document.getElementById("deldatacardid").value = val;
	...
	function callEdit() {
		...
		if (cnt == 0) {
			...
			if(isEligibleForOpertion(val)){
			document.getElementById("editdatacardid").value = val;
		...
		function callCreate() {
			document.forms[0].action = "createDataCard.action";
			document.forms[0].submit();
		}
		$(document).ready(function() {
			   window.history.pushState("","", "datacardlist.action");
			 });
...
<form name="form1" id="form1" action="ModifyDataCardDetails" method="POST">
	<input type="hidden" name="editdatacardid" id="editdatacardid" value="" />

</form>
<form name="form2" id="form2" action="DeleteDatacardDetails.action"
	method="POST">
	<input type="hidden" name="deldatacardid" id="deldatacardid" />
</form>
<%}else{%>
