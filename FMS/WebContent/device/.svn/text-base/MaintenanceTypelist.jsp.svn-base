				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>View Maintenance Type
							</div>
							<div class="actions">
							...
							<%}%>
								<%if(delete.equalsIgnoreCase("Y")){ %>
								 <a href="javascript:void(0)" class="btn red" id="deletemaintenancetype" onclick="calldelete()">
							...
							<div class="btn-group">
									...
									<ul class="dropdown-menu pull-right ">
										...
										<%if(status.equalsIgnoreCase("ACTIVE")){ %>
											<a class="btn blue" href="javascript: void(0)" onclick="window.open('','_new').location.href='/its/generateReport?id=MAINTENANCETYPERPT'">
						...
						<div class="portlet-body">
							...
							<s:if test="%{deletestatus=='fail'}">
								Could Not Delete Maintenance Type Deleted 
							</s:if>
							</div>
							<table class="table table-striped table-bordered table-hover"
								id="listmaintenancetype">
								<thead>
									<tr>
...
<%}%>
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you Sure, you want to delete this record?",
							function(result) {
								...
								if (result == true) {
									document.getElementById("deltypeid").value = val;
	...
	function callEdit() {
		...
		if (cnt == 0) {
			...
			if(isEligibleForOpertion(val)){
			document.getElementById("edittypeid").value = val;
		...
		function callCreate() {
			document.forms[0].action = "createMaintenanceType.action";
			document.forms[0].submit();
		}
		$(document).ready(function() {
			   window.history.pushState("","", "maintenancetypelist.action");
			 });
...
<form name="form1" id="form1" action="ModifyMaintenanceTypeDetails" method="POST">
	<input type="hidden" name="edittypeid" id="edittypeid" value="" />

</form>
<form name="form2" id="form2" action="DeleteMaintenanceTypeDetails.action"
	method="POST">
	<input type="hidden" name="deltypeid" id="deltypeid" />
</form>
<%}else{%>
