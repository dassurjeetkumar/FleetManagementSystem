				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>View RFID Card 
							</div>
							<div class="actions">
							...
							<%}%>
								<%if(delete.equalsIgnoreCase("Y")){ %>
								 <a href="javascript:void(0)" class="btn red" id="deleterfidcard" onclick="calldelete()">
							...
							<div class="btn-group">
									...
									<ul class="dropdown-menu pull-right ">
										...
										<%if(status.equalsIgnoreCase("ACTIVE")){ %>
											<a class="btn blue" href="javascript: void(0)" onclick="window.open('','_new').location.href='/its/generateReport?id=RFIDCARDRPT'">
						...
						<div class="portlet-body">
							...
							<s:if test="%{deletestatus=='fail'}">
								Could Not Delete RFID Card Deleted 
							</s:if>
							</div>
...
<%}%>
		...
		if (cnt == 0) {
			...
			bootbox.confirm("Are you Sure, you want to delete this record?",
							function(result) {
								...
								if (result == true) {
									document.getElementById("delrfidcardid").value = val;
	...
	function callEdit() {
		...
		if (cnt == 0) {
			...
			if(isEligibleForOpertion(val)){
			document.getElementById("editrfidcardid").value = val;
		...
		function callCreate() {
			document.forms[0].action = "createRFIDCard.action";
			document.forms[0].submit();
		}
		$(document).ready(function() {
			   window.history.pushState("","", "rfidcardlist.action");
			 });
...
<form name="form1" id="form1" action="ModifyRFIDCardDetails" method="POST">
	<input type="hidden" name="editrfidcardid" id="editrfidcardid" value="" />

</form>
<form name="form2" id="form2" action="DeleteRFIDcardDetails.action"
	method="POST">
	<input type="hidden" name="delrfidcardid" id="delrfidcardid" />
</form>
<%}else{%>
